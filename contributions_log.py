...
# OWNER_DID KHÔNG hardcode (fork-safe): mặc định RỖNG -> DID chủ không nằm trong code fork.
# Chủ nhận diện qua repo gốc trên Actions (2); muốn nhận diện ngoài Actions thì đặt env OWNER_DID.
...
IS_OWNER = (bool(OWNER_DID) and _my_did == OWNER_DID) or (_gh_repo == OWNER_REPO)
